		...
		this.each(function () {
			...
			if(typeof(options) == 'object') {
				if(typeof(options.ranges) != 'object') {
					...
					var day = now.getDate();
					options.ranges['上周'] = [moment().subtract('days', 6), moment()];
//					options.ranges['二周内'] = [moment().subtract('days', 13), moment()];
					options.ranges['上月'] = [moment().subtract('days', 29), moment()];
                                        options.ranges['今天'] = [new Date(year, month, day, 0, 0, 0), moment()];
                                        options.ranges['下周'] = [moment(), moment().subtract('days', -6)];
//					options.ranges['二周'] = [ moment(), moment().subtract('days',-13)];
					options.ranges['下月'] = [moment(), moment().subtract('days', -29)];
				}
				if(typeof(options.locale) != 'object') {
					options.locale = {
						applyLabel: "确定",
