                ...
                <div style="margin-top:20px;">
                    <ul class="order-process clearfix">
                        ...
                        <p class="order-process-state">
                            <?php  if($item['delivery_status'] == 0) { ?>待配送<?php  } ?>
                            <?php  if($item['delivery_status'] == 1) { ?>配送中<?php  } ?>
                            <?php  if($item['delivery_status'] == 2) { ?>已配送<?php  } ?></p>
                        <p class="bar"><i class="square finish">√</i></p>
                        <p class="order-process-time">
                            <?php  if(!empty($item['delivery_finish_time'])) { ?>
                            <?php  echo date('Y-m-d H:i:s', $item['delivery_finish_time'])?>
                            <?php  } ?>
                        </p>
                        </li>
                        <?php  } ?>
                    ...
                    <div class="panel-body clearfix">
                        <form class="form-horizontal form" action="" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-xs-12 col-sm-6">
                                ...
                                <div class="parting-line"></div>
                                <div class="form-group clearfix">
                                    <label class="col-xs-3 col-sm-3 control-label">店家备注：</label>
        ...
        $(".selectDish").click(function () {
            $.ajax({
                ...
                success: function (meg) {
                    var data = eval("(" + meg + ")");
                    /*
                    var string = "<tr><th style=\"width:16%;text-align: center\">商品名称</th><th style=\"width:8%;text-align: center\">图片</th><th style=\"width:8%;text-align: center\">价格</th><th style=\"width:6%;text-align: center\">打包费</th><th style=\"width:10%;text-align: center\">数量</th><th style=\"width:30%;text-align: center\">操作</th></tr>";
                    for (var i = 0; i < data.length; i++) {
                        string += "<tr><td align=\"left\">" + data[i].title + "</td><td align=\"center\"><img class='img-responsive' style='width:50px;' src='" + data[i].thumb + "'></td>";
                    ...
                    */
                    var string = "<tr><th style=\"width:26%;text-align: center\">商品名称</th><th style=\"width:8%;text-align: center\">图片</th><th style=\"width:8%;text-align: center\">价格</th><th style=\"width:8%;text-align: center\">打包费</th><th style=\"width:15%;text-align: center\">数量</th><th style=\"width:30%;text-align: center\">操作</th></tr>";
                    for (var i = 0; i < data.length; i++) {
                        string += "<tr><td align=\"left\">" + data[i].title + "</td><td align=\"center\"><img class='img-responsive' style='width:50px;' src='" + data[i].thumb + "'></td>";
                        //如果商品中有规格可选项，为保证name的唯一性，添加英文plus和optionid参数
                        if ( data[i].optionid != null ) {
                            string += "<td><input type='text' name='dish["+data[i].id+"plus"+data[i].optionid+"][price]' value='" + data[i].marketprice + "' style='width:30px;text-align:right;border:none;' readonly></td>";
                            string += "<td style=\"text-align:right\">" + data[i].packvalue + "</td>";
                            string += "<td><div class='input-group'><span class='input-group-addon' onclick='changeMinus(this)' >-</span><input type='text' name='dish[" + data[i].id +"plus"+data[i].optionid+"][num]' class='form-control dishNum text-center' value='0' ><span class='input-group-addon' onclick='changeAdd(this)'>+</span></div></td>";
                            if (data[i].status == 1) {
                                string += "<td><input data-value=0 name='dish[" + data[i].id +"plus"+data[i].optionid+"][status]'class='btn btn-danger' style='width:70px' onclick='changeDish(this)' value='选择' readonly></td>";
                            } else {
                                string += "<td><input data-value=0 style='width:70px' class='btn btn-danger disabled' value='己下架' readonly></td>";
                            }
                            string += "<td><input type='hidden' name='dish["+data[i].id+"plus"+data[i].optionid+"][optionid]' value='" + data[i].optionid + "' ></td>";
                            string += "<td><input type='hidden' name='dish["+data[i].id+"plus"+data[i].optionid+"][optionname]' value='" + data[i].optionname + "' ></td>";
                        } else {
                            string += "<td><input type='text' name='dish["+data[i].id+"plus"+"][price]' value='" + data[i].marketprice + "' style='width:30px;text-align:right;border:none;' readonly></td>";
                            string += "<td style=\"text-align:right\">" + data[i].packvalue + "</td>";
                            string += "<td><div class='input-group'><span class='input-group-addon' onclick='changeMinus(this)' >-</span><input type='text' name='dish[" + data[i].id+"plus"+"][num]' class='form-control dishNum text-center' value='0' ><span class='input-group-addon' onclick='changeAdd(this)'>+</span></div></td>";
                            if (data[i].status == 1) {
                                string += "<td><input data-value=0 name='dish[" + data[i].id+"plus"+"][status]'class='btn btn-danger' style='width:70px' onclick='changeDish(this)' value='选择' readonly></td>";
                            } else {
                                string += "<td><input data-value=0 style='width:70px' class='btn btn-danger disabled' value='己下架' readonly></td>";
                            }
                            string += "<td><input type='hidden' name='dish["+data[i].id+"plus"+"][optionid]' value='" + data[i].optionid + "' ></td>";
                            string += "<td><input type='hidden' name='dish["+data[i].id+"plus"+"][optionname]' value='" + data[i].optionname + "' ></td>";
                        }
                    }
                    string += "</tr>";
                    $("#addShow").html(string);
                ...
                <table class="table table-hover">
                    ...
                    <tr>
                        ...
                        <td><?php  if(!empty($category[$row['pcate']])) { ?><span class="text-error">[<?php  echo $category[$row['pcate']]['name'];?>] </span><?php  } ?>
                            <a href="<?php  echo $this->createWebUrl('goods', array('id' => $row['id'], 'op' => 'post', 'storeid' => $item['storeid']))?>"><?php  echo $row['title'];?></a>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        ...
                        <h4 class="modal-title">店家备注</h4></div>
                    <div class="modal-body">
