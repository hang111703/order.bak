...
if ($append == 2) {
    ...
    $dishInfo = pdo_fetchall("SELECT goodsid,optionid,optionname,price,total FROM " . tablename($this->table_order_goods) . " WHERE weid=:weid AND storeid=:storeid AND orderid=:orderid", array(":weid" => $weid, ":storeid" => $storeid, ":orderid" => $orderid));
    ...
    if ($rtype != 1) {
        foreach ($cart as $k => $v) {
            ...
            if (in_array($v['goodsid'], $dishid) && in_array($v['optionid'], $dishOptionid)) {
                $dishCon = array(":weid" => $weid, ":storeid" => $storeid, ":orderid" => $orderid, ":goodsid" => $v['goodsid'], ":optionid" => $v['optionid']);
                $sql = "UPDATE " . tablename($this->table_order_goods) . " SET total=total+{$v['total']},dateline=" . time() . " WHERE weid=:weid AND storeid=:storeid AND orderid=:orderid AND goodsid=:goodsid AND optionid = :optionid";
                ...
                if ( $v['optionid'] == 'null' ) {
                   ...
                   $parm = array("weid" => $weid, "storeid" => $storeid, "orderid" => $orderid, "goodsid" => $v['goodsid'], "price" => $v['price'], "total" => $v['total'], 'dateline' => time(), 'optionid' => $v['optionid'], 'optionname' => $v['optionname']);
                   pdo_insert($this->table_order_goods, $parm);
                }
            }
            ...
            //不考虑库存 
            if ($goodsName['counts'] == -1 ) {
                pdo_query("UPDATE " . tablename($this->table_goods) . " SET sales=sales+:counts,lasttime=:time WHERE id=:id", array(':id' => $v['goodsid'], ':counts' => $v['total'], ':time' => TIMESTAMP));
            } else {
