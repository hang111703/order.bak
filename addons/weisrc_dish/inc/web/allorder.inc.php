...
DESC LIMIT 1", array(':tid' => $id, ':uniacid' => $this->_weid));
    ...
    if ($_POST['selectDish']) {
        //考虑夜宵的情况
        if ( time() >= $start ) {
            $timetamp = " and find_in_set(dayofweek(now()) -1,week) > 0 "  ;
        } else {
            $timetamp = " and find_in_set(dayofweek(now()) -2,week) > 0 "  ; 
        }
        //如果有预订， 但暂不考虑凌晨预订这个特殊情况
        if ($order['dining_mode'] == 3) {
            $timetamp = " and find_in_set(dayofweek(DATE_FORMAT('" .$order['meal_time'] . "','%Y-%m-%d %H:%i')) -1,week) > 0 " ;
        }
        
        $dishCondition = '';
        //如果选择了商品分类，
        //原先没有考虑菜品按星期显示和是否当天有库存，暂没考虑预订那天的库存 -2018/07/01
        if ($_POST['addDishName']) {
            $dishCondition = "AND pcate=" . intval($_POST['addDishName']) . $timetamp . " and (counts > today_counts or counts = -1) ";
        } else {
            $dishCondition = $timetamp . " and (counts > today_counts or counts = -1) ";
        }
//        $allGoods = pdo_fetchall("SELECT * FROM " . tablename($this->table_goods) . " WHERE storeid=:storeid AND weid=:weid  AND deleted=0 " . $dishCondition, array(":storeid" => $storeid, ":weid" => $this->_weid));
        $allGoods = pdo_fetchall("SELECT * FROM ims_weisrc_dish_goods_view"  . " WHERE storeid=:storeid AND weid=:weid  AND deleted=0 " . $dishCondition, array(":storeid" => $storeid, ":weid" => $this->_weid));
...
//        $dishInfo = pdo_fetchall("SELECT goodsid,price,total FROM " . tablename($this->table_order_goods) . " WHERE weid=:weid AND storeid=:storeid AND orderid=:orderid", array(":weid" => $weid, ":storeid" => $storeid, ":orderid" => $id));
        $dishInfo = pdo_fetchall("SELECT goodsid,price,optionid,optionname,total FROM " . tablename($this->table_order_goods) . " WHERE weid=:weid AND storeid=:storeid AND orderid=:orderid", array(":weid" => $weid, ":storeid" => $storeid, ":orderid" => $id));
        foreach ($dishInfo as $v) {
            $dishid[] = $v['goodsid'];
            $dishOptionid[] = $v['optionid'];
        }
        foreach ($dish as $k => $v) {
            if ($v['status'] != "己选择" || empty($v['num'])) {
                ...
                if (!empty($dish)) {
                    //从$k中取出商品的id
                    if ( $v['optionid'] == 'null' ) {
                        $goodsidadd =substr($k,0,strlen($k) - 4);
                        $optionid = '';
                    } else {
                        $goodsidadd = substr($k,0,strlen($k) - strlen($v['optionid']) - 4);
                        $optionid = $v['optionid'] ;
                    }
                    //如果添加的菜品在已点好的菜单中
                    if (in_array($goodsidadd, $dishid) && in_array($optionid, $dishOptionid)) {
                        $dishCon = array(":weid" => $weid,":storeid"=>$storeid,":orderid"=>$id,":goodsid"=>$goodsidadd,":optionid"=>$optionid);
                        $sql="UPDATE ".tablename($this->table_order_goods)." SET total=total+{$v['num']},dateline=".time()." WHERE weid=:weid AND storeid=:storeid AND orderid=:orderid AND goodsid=:goodsid AND optionid=:optionid";
                        pdo_query($sql, $dishCon);
                    } else {
                         if ( $v['optionid'] == 'null' ) {
                            $parm = array("weid" => $weid, "storeid" => $storeid, "orderid" => $id, "goodsid" => $goodsidadd, "price" => $v['price'], "total" => $v['num'], 'dateline' => time());
                            pdo_insert($this->table_order_goods, $parm);
                         } else {
                            $parm = array("weid" => $weid, "storeid" => $storeid, "orderid" => $id, "goodsid" => $goodsidadd, "price" => $v['price'], "total" => $v['num'], 'dateline' => time(), 'optionid' => $v['optionid'], 'optionname' => $v['optionname']);
                            pdo_insert($this->table_order_goods, $parm);
                         }
                    }
                    //加菜对总销量和每日库存的影响 -2018/07/01
                     $goodsOne = pdo_fetch("SELECT * FROM " . tablename($this->table_goods) . " WHERE weid=:weid AND id=:goodsid ORDER by id
                                DESC LIMIT 1", array(':weid' => $this->_weid, ':goodsid' => $goodsidadd));
                    if ($goodsOne['counts'] == -1 || ($order['dining_mode'] == 3 && date('Y-m-d') <> substr($order['meal_time'],0,10))) {
                        //不考虑每日库存或者不是当天的预订,加菜仅造成总销量增加
                        pdo_update($this->table_goods, array('sales' => $goodsOne['sales'] + $v['num']), array('weid' => $this->_weid, 'id' => $goodsidadd));
                    } else {
                        //加菜造成总销量增加和每日库存减少 
                        if (($order['dateline'] >= $start && $order['dateline'] <= $end) || ($order['dining_mode'] == 3 && date('Y-m-d') == substr($order['meal_time'],0,10)) ) {
                            //当天的加菜造成总销量增加和每日库存减少
                            pdo_update($this->table_goods, array('today_counts' => $goodsOne['today_counts'] + $v['num'], 'sales' => $goodsOne['sales'] + $v['num']), array('weid' =>
                                $this->_weid, 'id' => $goodsidadd));
                        } else {
                            //不是当天的加菜，只造成总销量增加
                            pdo_update($this->table_goods, array('sales' => $goodsOne['sales'] + $v['num']), array('weid' => $this->_weid, 'id' => $goodsidadd));
                        }
                    ...
                    $add_goods = pdo_fetch("SELECT * FROM " . tablename($this->table_goods) . " WHERE weid=:weid AND id=:id ORDER by id DESC LIMIT 1", array(':weid' => $this->_weid, ':id' => $goodsidadd));
...
//    $goods = pdo_fetchall("SELECT a.goodsid,a.price, a.total,b.thumb,b.title,b.id,b.pcate,b.credit,a.optionname FROM " . tablename($this->table_order_goods) . " a INNER JOIN " . tablename($this->table_goods) . " b ON a.goodsid=b.id WHERE a.orderid = :id", array(':id' => $id));
      $goods = pdo_fetchall("SELECT a.goodsid,a.price, b.credit, a.total,b.thumb,b.title,b.id ,b.pcate,a.optionid,a.optionname FROM " . tablename($this->table_order_goods) . " a INNER JOIN " . tablename($this->table_goods) . " b ON a.goodsid=b.id WHERE a.orderid = :id", array(':id' => $id));
