...
class weisrc_dishModuleProcessor extends WeModuleProcessor {
	...
	public function respond() {
        ...
        if (empty($mc['nickname']) || empty($mc['avatar']) || empty($mc['resideprovince']) || empty($mc['residecity']) || empty($mc['gender'])) {
            ...
            mc_update($curopenid, array('nickname' => $mc['nickname'], 'avatar' => $mc['avatar'], 'resideprovince' => $mc['resideprovince'], 'residecity' => $mc['residecity'], 'gender' => $mc['gender']));
            $mc = mc_fetch($curopenid);
        }
        ...
        if ($this->message['msgtype'] == 'event') {
            ...
            if ($agent) {
                ...
                if (empty($fans)) {
                    ...
                    $checkmsg = $mc['nickname'] . "成为您的下级成员！";
                    $this->postText($agent['from_user'], $checkmsg);
                }
            } else {
                $checkmsg = "代理商不存在！" . $scene_str;
